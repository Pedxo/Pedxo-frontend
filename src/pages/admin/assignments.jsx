...
import {
  getAssignedTalentIds,
  isContractCompleted,
  getContractStatus,
  getMissingContractFields,
  isContractDataComplete,
} from "../../utility/contractUtils.js";
...
export default function AssignmentPage() {
  ...
  async function loadAll() {
    ...
    console.log("LOADED CONTRACTS:", sortedContracts);
    console.log("SAMPLE CONTRACT:", JSON.stringify(sortedContracts[0], null, 2));
...
const assignedContracts = contracts.filter(
  (c) => getContractStatus(c) === "assigned"
);

const completedContracts = contracts.filter(
  (c) => getContractStatus(c) === "completed"
);
  ...
  const openAssignModal = (contract) => {
    const missing = getMissingContractFields(contract);
    if (missing.length > 0) {
      setNotice({
        type: "error",
        text: `Contract is missing required info: ${missing.join(", ")}`,
      });
      return;
    }
  ...
  }; // ← function ends right here now, nothing after the finally block
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
        <div className="flex gap-2 flex-wrap">
          {[
            ...
            { key: "completed", label: "Ready to Assign", count: completedContracts.length },
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 text-xs text-gray-500">
                        {contract.paymentFrequency ? `${contract.paymentFrequency} · ${formatCurrency(contract.minimumToPayToTalent || contract.paymentRate)}` : `Budget: ${formatCurrency(contract.minimumToPayToTalent || contract.paymentRate)}`}
                      </div>
                      {!isContractDataComplete(contract) && (
                        <div className="mt-2 inline-block text-xs px-2 py-0.5 rounded bg-yellow-100 text-yellow-800">
                          Missing: {getMissingContractFields(contract).join(", ")}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex-shrink-0 flex flex-col items-end gap-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        {getContractStatus(contract) !== "pending" && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setSelectedContract(null)}>
          <div className="bg-white rounded-xl shadow-xl max-w-3xl w-full max-h-[85vh] overflow-y-auto relative p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              {getContractStatus(selectedContract) !== "pending" && (
